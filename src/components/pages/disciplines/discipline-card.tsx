...
import { Calendar, Flag, Pencil, Trash2, AlertTriangle } from "lucide-react";
			...
			<div className="p-6">
				...
				<div className="mt-6 border-t border-gray-100 pt-4 flex flex-col sm:flex-row items-stretch sm:items-center sm:justify-between gap-4">
					...
					<div className="flex items-center space-x-2 flex-shrink-0 mt-4 sm:mt-0">
						<Link href={`/disciplines/edit/${discipline._id}`} className="inline-flex items-center justify-center w-full sm:w-auto px-4 py-2 rounded-lg bg-gray-100 text-gray-700 font-semibold text-sm hover:bg-gray-200 hover:text-black transition-colors"><Pencil size={14} className="mr-2" />Edit</Link>
