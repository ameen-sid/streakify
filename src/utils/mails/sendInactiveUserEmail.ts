								...
								<table border="0" cellpadding="0" cellspacing="0" width="100%" style="max-width: 600px" class="container">
											...
											<table border="0" cellpadding="0" cellspacing="0" width="100%">
												...
												<tr>
													<td align="center" style="padding-bottom: 30px; font-family: Arial, sans-serif; font-size: 16px; line-height: 24px; color: #4b5563;">
														It's been a little while since we last saw you. Your goals are waiting! Remember that consistency is the key to building discipline.
													</td>
												</tr>
														...
														<table border="0" cellspacing="0" cellpadding="0">
															<tr>
																<td align="center" style="border-radius: 8px;" bgcolor="#111827">
																	<a href="${loginPageLink}" target="_blank" style="font-size: 18px; font-family: Arial, sans-serif; color: #ffffff; text-decoration: none; border-radius: 8px; padding: 15px 25px; border: 1px solid #111827; display: inline-block; font-weight: bold;">Get Back on Track</a>
																</td>
															</tr>
														</table>
													</td>
									...
									<tr>
										<td align="center" style="padding: 30px 20px; font-family: Arial, sans-serif; font-size: 12px; line-height: 18px; color: #9ca3af;">
											You are receiving this email as a gentle reminder. You can manage your email preferences in your account settings. <br /><br /> &copy; 2025 Discipline Planner. All rights reserved.
										</td>
									</tr>
								</table>
							</td>
...
const sendInactiveUserEmail = async (
	...
	try {
		...
		const title = EMAIL_SUBJECTS.INACTIVE_USER;
		const body = inactiveUserTemplate(username, loginPageLink);
		...
		console.log("Inactive User Email sent successfully to: ", email);
	} catch(error) {

		console.error("Error while sending inactive user email: ", error);
		throw new APIError(HTTP_STATUS.INTERNAL_SERVER_ERROR, "Failed to send inactive user email");
	}
};
