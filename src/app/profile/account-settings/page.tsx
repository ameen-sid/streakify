            ...
            <div className="bg-white rounded-2xl shadow-lg w-full max-w-md p-8">
                ...
                <div className="mt-6 flex justify-end gap-4">
                    ...
                    <button
                        ...
                        className="px-6 py-2 rounded-lg text-white bg-red-600 hover:bg-red-700 font-semibold disabled:bg-red-300 disabled:cursor-not-allowed"
                    >
                        Delete Account
                    </button>
                </div>
...
const AccountSettingsPage = () => {
    ...
    const handleAccountDeletion = async () => {
		...
		try {
			...
			const response = await axios.delete("/api/v1/profile/account");
			console.log("Account Deletion Status: ", response);

			setIsModalOpen(false);
			toast.success("Account deletion confirmed", { id: toastId });
			...
			if (axios.isAxiosError(error)) {
				...
				console.error("Account Deletion Fetch Failed: ", error.message);
                toast.error(error.message, { id: toastId });
            } else {
                    
				console.error("Account Deletion Fetch Failed: ", String(error));
