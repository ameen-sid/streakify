...
const CreateDisciplinePage = () => {
    ...
    const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
        ...
        e.preventDefault();
        setLoading(true);
        const toastId = toast.loading(`Creating new discipline: ${discipline}`);
		try {

			const response = await axios.post("/api/v1/disciplines", discipline);
			console.log("Discipline Creation Status: ", response);
			...
			if (axios.isAxiosError(error)) {
                ...
                toast.error(error.response?.data?.message || "Failed to create discipline", { id: toastId });
			} else if (error instanceof Error) {
